  ...
  return (
    <div className="min-h-screen bg-[#FAF9F6] px-4 py-8 text-slate-950">
      ...
      >
        ...
        <GradientText as="h1" className="mt-4 font-black text-4xl">Create Account</GradientText>
        <p className="mt-1 text-sm font-medium text-slate-700">Start your bold finance simulator journey.</p>
        <div className="mt-5 space-y-4">
          <TextField label="Full Name" fullWidth {...register("fullName", { required: true })} sx={{ "& .MuiOutlinedInput-root": { borderRadius: "16px", background: "#FAF9F6", border: "3px solid #3b2f54", boxShadow: "5px 5px 0px 0px rgba(249,168,212,0.12)", "& fieldset": { borderColor: "#3b2f54" }, "&:hover fieldset": { borderColor: "#7C3AED" }, "&.Mui-focused fieldset": { borderColor: "#7C3AED" } } }} />
          ...
          <TextField label="Password" fullWidth type="password" {...register("password", { required: true, minLength: 6 })} sx={{ "& .MuiOutlinedInput-root": { borderRadius: "16px", background: "#FAF9F6", border: "3px solid #3b2f54", boxShadow: "5px 5px 0px 0px rgba(249,168,212,0.12)", "& fieldset": { borderColor: "#3b2f54" }, "&:hover fieldset": { borderColor: "#7C3AED" }, "&.Mui-focused fieldset": { borderColor: "#7C3AED" } } }} />
        </div>
        <CuteButton type="submit" disabled={submitting} className="mt-4 flex w-full items-center justify-center gap-2 rounded-full border-[3px] border-[#3b2f54] bg-[#7C3AED] px-5 py-3 font-black text-white shadow-[6px_6px_0px_0px_rgba(124,58,237,0.18)] transition-all duration-200 hover:-translate-y-1 hover:bg-[#8b5cf6]">
          <FaMagic /> {submitting ? "Creating account..." : "Signup"}
        </CuteButton>
        <p className="mt-4 text-sm font-medium text-slate-700">
          Already have an account? <Link to="/login" className="font-black text-[#7C3AED] underline decoration-[#F9A8D4] decoration-[4px] underline-offset-4">Login</Link>
        </p>
      </motion.form>
    </div>
  );
